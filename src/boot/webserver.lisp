...
(defun webserver::write-html (html stream)
   (let ((tag-name (symbol-name (car html))))
      ...
      (multiple-value-bind (properties elements)
            (webserver::cut-html (cdr html))
         (dolist (property properties)
            (io:uformat stream " ~a=\"~a\"" (symbol-name (car property)) (cdr property)))
         (io:uformat stream ">")
         (dolist (element elements)
            (cond ((listp element) (webserver::write-html element stream))
                  (t               (io:uformat stream "~a" element)))))
      (io:uformat stream "</~a>" tag-name)))
