...
void LMud_Process_Create(struct LMud_Process* self, struct LMud_Fiber* fiber, struct LMud_Process** slot)
{
    self->slot      = slot;
    self->fiber     = fiber;
...
{
    return LMud_Profile_GetName(LMud_Fiber_GetProfile(LMud_Process_GetFiber(self)));
}
