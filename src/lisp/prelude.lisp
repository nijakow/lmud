...
(lambda ()
   ...
   (tos:defmethod (io.reader:<reporter> report-comment) (data &key from to) nil)
   ...
   (defun io.reader:read-multiline-comment (stream)
      (while t
         (cond ((io:eof-p stream) (io.reader:eof-error stream))
               ((io.reader:checkstr stream "#|")
                (io.reader:read-multiline-comment stream))
               ((io.reader:checkstr stream "|#") (return))
               (t (read-char stream)))))
      ...
      (let ((start (.text-position stream)))
         (cond ((io:eof-p stream)
               ...
               ((io.reader:checkstr stream ";")
                ...
                (.report-comment (.reporter meta) nil :from start :to (.text-position stream))
